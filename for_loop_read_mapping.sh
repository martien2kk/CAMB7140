...
# ---------- CONDITIONAL FOR-LOOP (subsampled FASTQs only) ----------
...
for fq in *_subsample.fastq.gz; do
  srr="${fq%%_*}"

  if [[ -n "${SAMPLE_MAP[$srr]+x}" ]]; then
    sample="${SAMPLE_MAP[$srr]}"
    echo "Mapping $fq -> $sample"
    kallisto quant -i Homo_sapiens.GRCh38.cdna.all.index \
      -o "$sample" -t 4 --single -l 250 -s 30 "$fq" &> "$sample.log"
  fi
done
